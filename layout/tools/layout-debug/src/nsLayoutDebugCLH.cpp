...
nsLayoutDebugCLH::Handle(nsICommandLine* aCmdLine) {
  ...
  rv = HandleFlagWithOptionalArgument(aCmdLine, NS_LITERAL_STRING("capture-profile"),
                                      profileFilename, captureProfile);
  NS_ENSURE_SUCCESS(rv, rv);

  if (profileFilename.IsVoid()) {
    profileFilename.AssignLiteral("profile.json");
  }
